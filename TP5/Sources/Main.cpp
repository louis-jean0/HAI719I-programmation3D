...
struct Light {
    ...
    void setupCameraForShadowMapping(glm::vec3 scene_center , float scene_radius) {
        ...
        // Projection matrix
        glm::vec3 upperBound = scene_center + glm::vec3(scene_radius);
        glm::vec3 lowerBound = scene_center - glm::vec3(scene_radius);
        glm::mat4 projectionMatrix = glm::ortho(lowerBound.x,upperBound.x,lowerBound.y,upperBound.y,0.1f,2*scene_radius);
...
struct Scene{
    ...
    void render() {
        ...
        // Then open shadow maps and send their coordinates to the shader
...
void initScene (const std::string & meshFilename) {
    ...
    // Setup lights :
    {
        ...
        }
        {
            ...
            Light & newLight = scene.scene_lights[ scene.scene_lights.size() - 1 ];
            //newLight.m_position = glm::vec3 (-2*cos(app_timer), 2*sin(app_timer), 3.5);
            newLight.m_position = glm::vec3(1.0,0.0,0.0);
            newLight.m_color = glm::vec3(1.0, 0.0, 0.0);
            newLight.m_intensity = 0.5f;
            newLight.allocateShadowMapFBO(shadow_map_width , shadow_map_height);
            newLight.shadowMapTexOnGPU = texture_slot_available;   ++texture_slot_available;
        }
        {
            ...
            //newLight.m_position = glm::vec3 (-2*cos(app_timer), 2*sin(app_timer), 3.5);
            newLight.m_position = glm::vec3(-1.0,0.0,0.0);
            newLight.m_color = glm::vec3(0.0, 0.0, 1.0);
            newLight.m_intensity = 0.5f;
            newLight.allocateShadowMapFBO(shadow_map_width , shadow_map_height);
            newLight.shadowMapTexOnGPU = texture_slot_available;   ++texture_slot_available;
        }
